from imports import Client, bigquery, Final, dask, Optional, Any, time, dask_bigquery
...
def main(request: Optional[Any]) -> str:
    ...
    data = api_call(
        access_token=access_token, look_id=2248, limit=10000, timeout=720
    )
...
def process_data(data, project_id, dataset_name, table_name, bq_client):
    ...
    # })
    start = time.time()
    bq_schema = transform_column_names(data)
    data.columns = bq_schema
    print(data.columns)
    credentials = bigquery.Client(project_id)._credentials
    if __name__ == '__main__':
        with dask.config.set(scheduler='processes'):
            ...
            # data = data.map_partitions(apply_func_on_df,
            #                            func_ton_run=upload_data_to_bq,
            #                            project_id=project_id,
            #                            dataset_name=dataset_name,
            #                            table_name=table_name,
            #                            data=data)
