...
def delete_from_table_if_exists():
    ...
    query_delete_job = client.query(
        ...
        select * from `xplenty.INFORMATION_SCHEMA.TABLES` where table_name='looker_pii_data_staging'

        )
        then
        delete `ex-nuts.xplenty.looker_pii_data_staging` where true;
        else
