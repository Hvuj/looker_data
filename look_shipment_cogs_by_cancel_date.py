...
path = 'shipments_data_by_cancel_date.csv'
...
def delete_from_table_if_exists():
    ...
    query_delete_job = client.query(
        ...
        select * from `xplenty.INFORMATION_SCHEMA.TABLES` where table_name='looker_daily_shipment_data_by_cancel_date_staging'

        )
        then
        delete `ex-nuts.xplenty.looker_daily_shipment_data_by_cancel_date_staging` where true;
        else
...
def run_task(request):
    ...
    table_name = 'looker_daily_shipment_data_by_cancel_date'
    ...
    dask_data = get_looker_data(access_token=get_looker_access_token(client_id=CLIENT_ID,
                                                                     client_secret=CLIENT_SECRET), look_id=2360,
                                limit=row_limit)
